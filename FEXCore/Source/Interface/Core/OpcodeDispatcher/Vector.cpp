...
Ref OpDispatchBuilder::VPDPBUSDOpImpl(IR::OpSize Size, Ref Acc, Ref Src1, Ref Src2, bool Saturating) {
  // Does four unsigned * signed byte multiplies per 32-bit element, sums them and accumulates in to the destination
  //
  // x86 VPDPBUSD: xmm1, xmm2, xmm3
  //               xmm1[31:0]  += xmm2[7:0]*xmm3[7:0] + xmm2[15:8]*xmm3[15:8] + xmm2[23:16]*xmm3[23:16] + xmm2[31:24]*xmm3[31:24]
  //               xmm1[63:32] += xmm2[39:32]*xmm3[39:32] + xmm2[47:40]*xmm3[47:40] + xmm2[55:48]*xmm3[55:48] + xmm2[63:56]*xmm3[63:56]
  //               etc.. for larger registers
  //
  // The sum of four products can't overflow 32-bits, so only the accumulate saturates for VPDPBUSDS.
  ...
  // Widen to 16-bit, multiply to 32-bit, then pairwise add twice to reduce each group of four products to one.
...
Ref OpDispatchBuilder::VPDPWSSDOpImpl(IR::OpSize Size, Ref Acc, Ref Src1, Ref Src2, bool Saturating) {
  // PMADDWD accumulated in to the destination.
  ...
  // The only overflowing dot product is (-32768 * -32768) * 2 = 2^31, which PMADDWD wraps to INT_MIN.
  // saturate(Acc + 2^31) == saturate(Acc - INT_MIN), so fix those elements up with a saturating subtract.
  // Only INT_MIN and zero equal their own negation, and zero gives Acc either way.
